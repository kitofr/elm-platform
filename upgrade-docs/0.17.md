...
### Port modules
For house keeping sake there's also a new concept called `port module`. All ports must be defined inside a `port module` and the recommendation is to stick to one per project. 
A `port module` is declared as follows:

```elm
port module Ports exposing (..)
```

If you forget to do this, Elm provides you with a friendly compiler message:

```
-- BAD PORT ------------------------------------------------------- src/Main.elm

You are declaring port `focus` in a normal module.

30| port focus : String -> Cmd msg
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
-- note: the implementation of focus is removed.
```

Instead of hooking up a signal, you have a function that can create commands. So you just call `focus : String -> Cmd msg` in your update function as part of the return tuple. That way the command is processed and sent through the port.
See example [here](http://guide.elm-lang.org/interop/javascript.html)
